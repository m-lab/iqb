...
class IQBCache:
    ...
    def get_data_from_parquet(
        ...
        """
        Fetch measurement data from Parquet cache (experimental).

        This method loads data from Parquet files generated by iqb_bigquery_fetch(),
        which uses a content-addressable directory structure. This is an alternative
        to get_data() for A/B testing performance with different storage formats.

        Args:
            country: ISO 2-letter country code (e.g., "US", "DE", "BR").
            start_date: Start of date range (inclusive).
            end_date: End of date range (exclusive).
            aggregation: Aggregation level ("country", "country_asn", "country_city_asn", etc.).
            percentile: Which percentile to extract (1-99).
        ...
        Raises:
            FileNotFoundError: If requested data is not available in cache.
            ValueError: If country not found in aggregated data.

        Example:
            >>> cache = IQBCache(cache_dir="data/cache/v1")
            >>> data = cache.get_data_from_parquet(
            ...     country="US",
            ...
            ...     percentile=95,
            ... )
            >>> print(data["m-lab"]["download_throughput_mbps"])
        """
        # Construct content-addressable path
        start_ts = start_date.strftime("%Y-%m-%dT00:00:00Z")
        end_ts = end_date.strftime("%Y-%m-%dT00:00:00Z")

        data_dir = self.cache_dir / start_ts / end_ts / aggregation

        downloads_file = data_dir / "downloads.parquet"
        uploads_file = data_dir / "uploads.parquet"

        # Check if files exist
        if not downloads_file.exists():
            raise FileNotFoundError(
                f"No cached downloads data found: {downloads_file}. "
                f"Run iqb_bigquery_fetch() to populate cache."
            )

        if not uploads_file.exists():
            raise FileNotFoundError(
                f"No cached uploads data found: {uploads_file}. "
                f"Run iqb_bigquery_fetch() to populate cache."
            )

        # Load Parquet files with filtering
        country_upper = country.upper()

        # Determine which columns to read based on percentile
        p_suffix = f"_p{percentile}"
        columns = ["country_code", f"download{p_suffix}", f"latency{p_suffix}", f"loss{p_suffix}"]

        downloads_df = pd.read_parquet(
            downloads_file,
            filters=[("country_code", "==", country_upper)],
            columns=columns,
        )

        uploads_df = pd.read_parquet(
            uploads_file,
            filters=[("country_code", "==", country_upper)],
            columns=["country_code", f"upload{p_suffix}"],
        )

        # Check if country exists in data
        if len(downloads_df) == 0:
            raise ValueError(
                f"Country '{country_upper}' not found in cached data. "
                f"Check that data exists for this country in {data_dir}"
            )

        # Extract values (assuming single row for country-level aggregation)
        download_row = downloads_df.iloc[0]
        upload_row = uploads_df.iloc[0]

        # Return in format expected by IQBCalculator
        return {
            "m-lab": {
                "download_throughput_mbps": float(download_row[f"download{p_suffix}"]),
                "upload_throughput_mbps": float(upload_row[f"upload{p_suffix}"]),
                "latency_ms": float(download_row[f"latency{p_suffix}"]),
                "packet_loss": float(download_row[f"loss{p_suffix}"]),
            }
        }
