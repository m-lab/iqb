...
class IQB:
    ...
    def set_config(self, config):
        ...
        if config is None:
            ...
            """
            raise NotImplementedError("method for reading from configuration file other than the default not implemented")
    ...
    def calculate_binary_requirement_score(self, network_requirement, value, threshold):
        ...
        if network_requirement == 'download throughput':
            ...
            raise ValueError(f'The binary requirement score method is not implemented for the network_requirement: {network_requirement}') 
    ...
    def calculate_iqb_score(self, data=None, print_details=False):
        ...
        if data is None:
            ...
            raise NotImplementedError("Method for calculating IQB score given a dataset is not implemented")
        ...
        for uc in self.config['use cases']:
            ...
            for nr in self.config['use cases'][uc]['network requirements']:
                ...
                for ds in self.config['use cases'][uc]['network requirements'][nr]['datasets']:
                    ds_w = self.config['use cases'][uc]['network requirements'][nr]['datasets'][ds]['w']
                    if ds_w > 0:
                        brs = self.calculate_binary_requirement_score(nr, measurement_data[ds][nr], nr_th) # binary requirement score (dataset, network requirement)
                        ds_S.append(brs)
                        print(f'Binary score: {uc},{nr},{ds},{nr_th},{measurement_data[ds][nr]}-->{brs}') if print_details else None
                ras = sum(ds_S)/len(ds_S) # requirement agreement score (all datasets for this requirement)
            ...
            ucs = sum(nr_Scores)/sum(nr_Weights) # use case score (all requirements for this use case)
            print(f'\t\t Net requirement score: {nr_Scores},{nr_Weights}-->{ucs}\n') if print_details else None 
            uc_Scores.append(ucs*uc_w)
            uc_Weights.append(uc_w)

        iqb_score = sum(uc_Scores)/sum(uc_Weights)
        print(f'\t\t\t IQB score: {uc_Scores},{uc_Weights}-->{iqb_score}') if print_details else None
